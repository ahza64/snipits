...
if (require.main === module) {
  ...
  baker.command(function (){
    authorizeUser(listFiles);
  }, {command: "listFiles"});
  
  baker.command(function(fileName){
    authorizeUser(function(auth){
      var service = google.drive('v3');
      var create_file = BPromise.promisify(service.files.create);
      var create_permission = BPromise.promisify(service.permissions.create);
      ...
      co(function*(){
        ...
        for(var i = 0; i < USERS.length; i++) {
          permission.resource.emailAddress = USERS[i];
...
function authorizeUser(callback, secret_file) {
  secret_file = secret_file || 'client_secret.json';
