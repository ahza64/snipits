...
function *run(circuit_name){
  ...
  for(var s = 0; s < folder.services.length; s++){
    ...
    for(var i = 0 ; i < service.layers.length; i++) {
      ...
      if(layer.name.indexOf(circuit_name) !== -1 && layer.name.endsWith("TreeTops")) {
        console.log("FOUND LAYER", circuit_name, layer);
        yield checkTrees(circuit_name, layer, base_url);
      }
...
function *checkTrees(circuit_name, layer, base_url) {
  ...
  var ingested = {count: 0, projects: {}, circuits: {}};
  var not_ingested = {count: 0, priority: {}, project: {}};
  
  
  for(var i = 0; i <= feature_ids.length; i+=batch_size){
    var ids = feature_ids.slice(i, i+batch_size);
    params.objectIds = ids.join(",");
    var features = yield http_get(url, params);
    for(var j = 0; j < features.features.length; j++) {
      var tree = features.features[j];
      var db_tree = yield Tree.findOne({qsi_id: tree.attributes.TREEID});
      
      if(db_tree) {
        ingested.count++;
        ingested.circuits[db_tree.circuit_name] = ingested.circuits[db_tree.circuit_name]|| 0;
        ingested.circuits[db_tree.circuit_name]++;
                
        ingested.projects[db_tree.pge_pmd_num] = ingested.projects[db_tree.pge_pmd_num] || 0;
        ingested.projects[db_tree.pge_pmd_num]++;
        console.log(i+j, "INGESTED",  tree.attributes.TREEID, db_tree.circuit_name, db_tree.pge_pmd_num);
      } else {
        not_ingested.count++;
        not_ingested.priority[tree.attributes.Priority] = not_ingested.priority[tree.attributes.Priority] || 0;
        not_ingested.priority[tree.attributes.Priority]++;
        not_ingested.project[tree.attributes.PMD_NUM] = not_ingested.project[tree.attributes.PMD_NUM] || 0;
        not_ingested.project[tree.attributes.PMD_NUM]++;
        console.log(i+j, "NOT INGESTED",  tree.attributes.TREEID, tree.attributes.PMD_NUM);        
      }
    }
    //       line.name = sanitizeLineName(span.attributes.LINE_NAME);
  }
  console.log("INGESTED", ingested);
  console.log("NOT INGESTED", not_ingested);
}
