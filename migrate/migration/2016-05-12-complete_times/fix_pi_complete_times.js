...
function *run(fix){
  ...
  for(var tree_id in updates) {
    if(updates.hasOwnProperty(tree_id)){
      ...
      if(!tree) {
      ...
      } else if(!tree.pi_complete_time && tree.status[0] !== '1') {
        var v2_tree = yield TreeV2.findOne({_id: tree_id});
        var v2_value = null;
        if(v2_tree && v2_tree.complete_time) {
          v2_value = v2_tree.complete_time;
        }
        ...
        if(update.value) {
          ...
          console.log("could not find pi_complete_time", tree_id);
        }
      }      
    }
  }
...
function *treesToUpdate() {
  var thistory = yield TreeHistory.find({action_field: "pi_complete_time"}).sort({created: 1, _id: 1});
  var updates = {};
...
function *fixStatus(tree, complete_time) {
  ...
  yield TreeHistory.create({
    action_code: "update",
    action_field: "pi_complete_time",
    action_value_from: tree.pi_complete_time,
    action_value_to: complete_time,
    object_type: "Tree",
    object_id: tree._id,
    performer_id: "fix_pi_complete_times.js",
    performer_type: "Migration Script",
    created: new Date()
  });
  
  tree.pi_complete_time = complete_time;
  yield tree.save();
}
