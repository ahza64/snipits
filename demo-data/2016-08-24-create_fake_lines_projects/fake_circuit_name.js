...
function *run(collection, field, push){
	...
	var tree_circuit_names = yield  model_name[collection].find().distinct('circuit_name').exec();
	console.log(tree_circuit_names);
	var circuit_names = [];
	tree_circuit_names.forEach((circuit_name,i) => circuit_names.push({
			"old": circuit_name,
			"new": 'line '+ i
		})
	);
